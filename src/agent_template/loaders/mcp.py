...
def load_mcp_tools(servers: list) -> list:
    tools = []
    for server in servers:
        if server.transport != "streamable-http":
            ...
            continue
        try:
            tools += run_sync(_fetch_tools(server))
...
async def _fetch_tools(server):
    client = MultiServerMCPClient(
        {server.name: {"transport": "streamable_http", "url": server.url}}
    )
