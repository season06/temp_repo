...
❯ conductor 的 tomcat busy thread 成長到 100% 沒有正常被釋放，通常可能是什麼原因
  跟上述 archive 沒有直接關連, 請依照 conductor 系統架構去思考

● Skill(superpowers:systematic-debugging)
Successfully loaded skill

Phase 1：先把「Tomcat thread 可能卡在哪裡」從程式碼裡找出來，而不是憑印象猜。

  Searched for 6 patterns, ran 7 shell commands

一個關鍵細節要確認——Jedis pool 耗盡時的等待行為。

  Searched for 2 patterns, ran 1 shell command

Phase 1 完成。查下來有一個結構性的具體嫌疑，而且它剛好能解釋「沒有正常被釋放」這個特徵。

---

架構前提：Conductor 的 REST 層不是薄層

大部分人直覺會把 Tomcat thread 當成「接請求、丟給後端」的薄層。Conductor 不是——編排引擎直接跑在 HTTP 請求執行緒上：

// WorkflowExecutorOps.java:884-886  ← updateTask 的最後
if (!isLazyEvaluateWorkflow(workflowInstance.getWorkflowDefinition(), task
    decide(workflowId);
}

而 isLazyEvaluateWorkflow（:961-986）的最後一行是：

return workflowTasks.stream().noneMatch(t -> t.getTaskReferenceName().equa
        && task.getStatus().isSuccessful();

一般 task 當然存在於 def 裡，noneMatch 是 false → 回傳 false → decide() 同步執行。lazy 只在 fork 分支被 JOIN 等待、或 task 不在 def 裡時才生效。

所以 worker 打一次 POST /api/tasks，在同一條 Tomcat 執行緒上會依序做完：

┌─────┬──────────────────────────────────────────────────────────────────────────────────────────────┬─────────────────────────┐
│  #  │                                             動作                          打到哪          │
├─────┼──────────────────────────────────────────────────────────────────────────────────────────────┼─────────────────────────┤
│ 1   │ getWorkflowModel + getTaskModel                                   MariaDB                 │
├─────┼──────────────────────────────────────────────────────────────────────────────────────────────┼─────────────────────────┤
│ 2   │ external payload 下載（若有設）                                   S3                      │
├─────┼──────────────────────────────────────────────────────────────────────────────────────────────┼─────────────────────────┤
│ 3   │ executionDAOFacade.updateTask                                     MariaDB + 同步 ES index │
├─────┼──────────────────────────────────────────────────────────────────────────────────────────────┼─────────────────────────┤
│ 4   │ addTaskExecLog（ExecutionDAOFacade，asyncIndexingEnabled=false 時 ES                      │
├─────┼──────────────────────────────────────────────────────────────────────────────────────────────┼─────────────────────────┤
│ 5   │ queueDAO.remove / postpone                                        Redis                   │
├─────┼──────────────────────────────────────────────────────────────────────────────────────────────┼─────────────────────────┤
│ 6   │ decide() → 取鎖、讀整個 workflow 含所有 task、評估、排程新 task、iMariaDB + Redis + ES    │
└─────┴──────────────────────────────────────────────────────────────────────────────────────────────┴─────────────────────────┘

Tomcat thread 耗盡在 Conductor 幾乎不是 thread leak，而是 200 條執行緒全部 park 在下游資源池上。

---

而那 200 條執行緒前面，擋著三個大小都是 10 的池

┌─────────────────────┬──────────────────────┬──────────────────────────────────────────────────────────────────────────────────┐
│         池          │         大小         │                                                     │
├─────────────────────┼──────────────────────┼──────────────────────────────────────────────────────────────────────────────────┤
│ Tomcat worker       │ 200                  │ Spring 預設（application.pr註解掉的，代表全用預設） │
├─────────────────────┼──────────────────────┼──────────────────────────────────────────────────────────────────────────────────┤
│ HikariCP（MariaDB） │ 10                   │ Spring 預設                                         │
├─────────────────────┼──────────────────────┼──────────────────────────────────────────────────────────────────────────────────┤                   │ Jedis（Redis）      │ 10                   │ RedisProperties:72 maxConne                         │
├─────────────────────┼──────────────────────┼──────────────────────────────────────────────────────────────────────────────────┤                   │ ES RestClient       │ 10 / route，30 total │ RestClient 預設，ElasticSea                         │
└─────────────────────┴──────────────────────┴──────────────────────────────────────────────────────────────────────────────────┘                   
200 : 10 : 10 : 10。任何一個下游變慢，執行緒堆積是必然結果，不是異常。                                                                              
---                                                                                                                                                 
頭號嫌疑：Jedis 連線池耗盡時會無限阻塞                                                                                                              
這條直接對應你說的「沒有正常被釋放」。                                                                                                              
// RedisStandaloneConfiguration.java:43-45                                                                                                          JedisPoolConfig config = new JedisPoolConfig();
config.setMinIdle(2);                                                                                                                               config.setMaxTotal(properties.getMaxConnectionsPerHost());   // 10
// ← 從頭到尾沒有 setMaxWaitMillis                                                                                                                  
而那個看起來就是為此存在的參數：                                                                                                                    
$ grep -rn "getMaxTimeoutWhenExhausted" --include=*.java .                                                                                          redis-persistence/.../DynomiteClusterConfiguration.java:44
                                                                                                                                                    maxTimeoutWhenExhausted（預設 800ms，RedisProperties:78）只被 Dynomite 那  redis_cluster、redis_sentinel三條路徑全都沒有套用——RedisClusterConfiguration:51 和 RedisSentinelConfiguration:50 也一樣只設了 setMaxTotal。                                      
Jedis 3.3.0（dependencies.gradle:41）的 JedisPoolConfig 建構子只覆寫 testWhileIdle / minEvictableIdleTime / timeBetweenEvictionRuns / numTestsPerEvimaxWait。繼承下來的 commons-pool2 預設是 blockWhenExhausted = true + maxWa
                                                                                                                                                    結論：一旦 10 條 Redis 連線全被借走，第 11 條之後的執行緒會在 GenericObjec待。
                                                                                                                                                    注意這裡有個很重要的區分——單一 Redis 操作是有超時的（Protocol.DEFAULT_TIME:51-73）。沒有超時的是「借連線」這個動作本身。所以只要有任何一個持有者卡住（網路抖動、Redis 主從切換、一個慢指令、或單純是併發超過 10），後面就會雪崩式堆積，而且負載退去也不會自己恢復。         
這是所有候選裡**唯一一個「不會自我修復」**的。你的症狀描述直接指向它。                                                                              
---                                                                                                                                                 
其他候選（會拖慢，但通常會恢復）                                                                                                                    
┌─────────────────┬───────────────────────────────────────────────────────────────────────────────────────┬─────────────────────────────────────────│      嫌疑       │                                         出處             │                               上限                                │
├─────────────────┼───────────────────────────────────────────────────────────────────────────────────────┼─────────────────────────────────────────│ Hikari          │ Spring 預設 10                                           │ connectionTimeout 30s → 拋例外，會恢復                            │
│ 連線池耗盡      │                                                                                       │                                         ├─────────────────┼──────────────────────────────────────────────────────────┼───────────────────────────────────────────────────────────────────┤
│ ES 全域 index   │ ElasticSearchRestDAOV7:1261 private synchronized void indexBulkRequest，且            │ 鎖跨越阻塞式 HTTP 呼叫持有 → 所有執行緒的│ 鎖              │ indexBatchSize 預設 1（ElasticSearchProperties:42）      │ 全域序列化，跟執行緒數無關                                        │
├─────────────────┼───────────────────────────────────────────────────────────────────────────────────────┼─────────────────────────────────────────│ ES socket 阻塞  │ RestClient 預設 socket timeout 30s；restClientConnecti   │ 30s/次，會恢復但很久                                              │
│                 │ -1（ElasticSearchProperties:67）等於沒生效                                            │                                         ├─────────────────┼──────────────────────────────────────────────────────────┼───────────────────────────────────────────────────────────────────┤
│ long-poll       │ ExecutionService:102 MAX_POLL_TIMEOUT_MS = 5000                                       │ 5s。要算容量：worker 數 × 平均 poll 佔用│ 正常佔用        │                                                          │                                                                   │
├─────────────────┼───────────────────────────────────────────────────────────────────────────────────────┼─────────────────────────────────────────│ 分散式鎖爭用    │ WorkflowExecutorOps:591 / :1249 用 acquireLock(id, 600   │ 若 workflow-execution-lock-enabled=true，terminate /              │
│                 │                                                                                       │ pauseWorkflow 可阻塞 60 秒              ├─────────────────┼──────────────────────────────────────────────────────────┼───────────────────────────────────────────────────────────────────┤
│ decide 本身慢   │ 大型 workflow：decide 要讀「整個 workflow + 所有 task」，JQ / INLINE task             │ 無上限                                  │                 │ 也在這條執行緒上跑                                       │                                                                   │
└─────────────────┴───────────────────────────────────────────────────────────────────────────────────────┴─────────────────────────────────────────
補一句：Tomcat 沒有請求層級的超時。上面任何一項只要下游不回，執行緒就一直被佔著。                                                                   
---                                                                                                                                                 
決定性證據：一份 thread dump 就能定案                                                                                                               
不要用改參數的方式試——直接在 busy thread 高的當下抓：                                                                                               
jcmd <pid> Thread.print > /tmp/dump1.txt                                                                                                            sleep 5 && jcmd <pid> Thread.print > /tmp/dump2.txt   # 抓兩份比對，確認是
                                                                                                                                                    # 看 http-nio 執行緒都停在哪
grep -A3 '"http-nio' /tmp/dump1.txt | grep "at " | sort | uniq -c | sort -rn | head -20                                                             
對照表：                                                                                                                                            
┌──────────────────────────────────────────────────────────┬──────────────────────┬─────────────────────────────────────────────────┐               │                      停在這個 frame                      │         根因   對策                       │
├──────────────────────────────────────────────────────────┼──────────────────────┼─────────────────────────────────────────────────┤               │ GenericObjectPool.borrowObject + redis.clients.jedis     │ Jedis 池耗盡                              │
├──────────────────────────────────────────────────────────┼──────────────────────┼─────────────────────────────────────────────────┤               │ HikariPool.getConnection                                 │ DB 池耗盡    rce.hikari.maximum-pool-size │
├──────────────────────────────────────────────────────────┼──────────────────────┼─────────────────────────────────────────────────┤               │ ElasticSearchRestDAOV7.indexBulkRequest（狀態 BLOCKED）  │ 全域 index 鎖，或關 taskIndexingEnabled   │
├──────────────────────────────────────────────────────────┼──────────────────────┼─────────────────────────────────────────────────┤               │ RestClient.performRequest / socket read（TIMED_WAITING） │ ES 慢        查 ES                        │
├──────────────────────────────────────────────────────────┼──────────────────────┼─────────────────────────────────────────────────┤               │ RedisDynoQueue.pop / ExecutionService.poll               │ long-poll 正                              │
├──────────────────────────────────────────────────────────┼──────────────────────┼─────────────────────────────────────────────────┤               │ ExecutionLockService.acquireLock                         │ 分散式鎖     -execution-lock-enabled      │
├──────────────────────────────────────────────────────────┼──────────────────────┼─────────────────────────────────────────────────┤               │ DeciderService / evaluate                                │ 編排本身慢                                │
└──────────────────────────────────────────────────────────┴──────────────────────┴─────────────────────────────────────────────────┘               
BLOCKED vs TIMED_WAITING vs WAITING 這三個狀態就能把範圍砍到剩一兩個。                                                                              
---                                                                                                                                                 
對應處置（依上面結果選）                                                                                                                            
1. Jedis 池（如果 dump 指向這裡——我認為機率最高）                                                                                                   
先把池開大到跟並發量匹配：                                                                                                                          
conductor.redis.maxConnectionsPerHost=100                                                                                                           conductor.redis.maxIdleConnections=50
                                                                                                                                                    但這只是提高門檻，不是修好——無限阻塞的行為還在。真正的修法是在 RedisStanda ：
                                                                                                                                                    JedisPoolConfig config = new JedisPoolConfig();
config.setMinIdle(2);                                                                                                                               config.setMaxTotal(properties.getMaxConnectionsPerHost());
config.setMaxWaitMillis(properties.getMaxTimeoutWhenExhausted().toMillis());  // ← 補這行                                                           
這讓 standalone 路徑跟 Dynomite 路徑（DynomiteClusterConfiguration:44）行為一致：池滿時快速失敗，worker 收到 5xx 重試，而不是把 Tomcat              執行緒吃光。把「無限阻塞」換成「快速失敗」，是這裡唯一能讓系統自我恢復的改
                                                                                                                                                    2. 給 ES 加 socket timeout，理由同前一輪——不能讓沒有超時的下游拖垮上游。
                                                                                                                                                    3. 重算 poll 的執行緒預算。 worker 數 × 每個 worker 的 poll 併發 就是穩態  緒數。如果這個數字接近 200，那就沒有餘裕處理 updateTask。兩個方向：把server.tomcat.threads.max 調到 400–500，或讓 worker 縮短 poll timeout / 減少併發。                                                                  
4. 加請求超時，避免單一卡住的請求永久佔用執行緒。                                                                                                   
5. 監控三個池的飽和度，這才是「壞掉之前會先漲」的指標：hikaricp_connections_pending、Jedis 的 active/idle、tomcat_threads_busy。現在的狀況是 Tomcat 執行緒數變成了唯一可見的指標，但它是最末端的症狀，看到它滿的時候真正的瓶頸

---

我沒有你的 thread dump，所以上面第 3 節是基於程式碼的最高機率假設，不是結 A3 '"http-nio' 的輸出就夠——我可以直接幫你定位到具體那一條。